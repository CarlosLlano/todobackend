# project variables
## ?= --> equals to PROJECT NAME env or todobackend if env not defined
...
# to stop make process whenever a docker command execution fails
INSPECT := $$(docker inspect -f {{.State.ExitCode}} $$(docker-compose -p $$1 -f $$2 ps -q $$3))
CHECK := @bash -c '\
	if [[ ${INSPECT} -ne 0 ]]; \
...
## @ -> does not display the executed command
test:
